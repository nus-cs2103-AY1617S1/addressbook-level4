...
* Only items that have a match with all input keywords will be shown (i.e. "AND" search) - e.g. `find fish ball` will list to-do item `fish for a ball` but not `fish for a fish`
