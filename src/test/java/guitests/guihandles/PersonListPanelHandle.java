...
public class PersonListPanelHandle extends GuiHandle {
    ...
    public List<ReadOnlyDatedTask> getSelectedPersons() {
        ListView<ReadOnlyDatedTask> personList = getListView();
        return personList.getSelectionModel().getSelectedItems();
    }

    public ListView<ReadOnlyDatedTask> getListView() {
        return (ListView<ReadOnlyDatedTask>) getNode(PERSON_LIST_VIEW_ID);
    }
     ...
     */
    public boolean isListMatching(ReadOnlyDatedTask... persons) {
     ...
     */
    public boolean containsInOrder(int startPosition, ReadOnlyDatedTask... persons) {
        List<ReadOnlyDatedTask> personsInList = getListView().getItems();
     ...
     * @param persons A list of person in the correct order.
     */
    public boolean isListMatching(int startPosition, ReadOnlyDatedTask... persons) throws IllegalArgumentException {
    ...
    public PersonCardHandle navigateToPerson(String name) {
        ...
        final Optional<ReadOnlyDatedTask> person = getListView().getItems().stream().filter(p -> p.getName().fullName.equals(name)).findAny();
        if (!person.isPresent()) {
     ...
     */
    public PersonCardHandle navigateToPerson(ReadOnlyDatedTask person) {
     ...
     */
    public int getPersonIndex(ReadOnlyDatedTask targetPerson) {
        List<ReadOnlyDatedTask> personsInList = getListView().getItems();
     ...
     */
    public ReadOnlyDatedTask getPerson(int index) {
