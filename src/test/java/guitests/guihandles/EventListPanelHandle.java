...
public class EventListPanelHandle extends GuiHandle {
    ...
    public ObservableList<Event> getSelectedtasks() {
        ListView<Event> taskList = getListView();
        return taskList.getSelectionModel().getSelectedItems();
    }

    public ListView<Event> getListView() {
        return (ListView<Event>) getNode(task_LIST_VIEW_ID);
    }
     ...
     */
    public boolean isListMatching(Event... tasks) {
     ...
     */
    public boolean containsInOrder(int startPosition, Event... tasks) {
        List<Event> tasksInList = getListView().getItems();
     ...
     * @param tasks A list of task in the correct order.
     */
    public boolean isListMatching(int startPosition, Event... tasks) throws IllegalArgumentException {
        ...
        for (int i = 0; i < tasks.length; i++) {
            ...
            guiRobot.sleep(200);
            if (!TestUtil.compareCardAndEvent(getEventCardHandle(startPosition + i), (Event) tasks[i])) {
                return false;
            }
        }
        return true;
    ...
    public TaskCardHandle navigateTotask(ReadOnlyTask readOnlyTask) {
        ...
        final Optional<Event> task = getListView().getItems().stream().filter(p -> p.getName().name.equals(readOnlyTask)).findAny();
        if (!task.isPresent()) {
     ...
     */
    public EventCardHandle navigateToevent(Event event) {
