...
public class DeadlineListPanelHandle extends GuiHandle {
    ...
    public ObservableList<Deadline> getSelectedtasks() {
        ListView<Deadline> taskList = getListView();
        return taskList.getSelectionModel().getSelectedItems();
    }

    public ListView<Deadline> getListView() {
        return (ListView<Deadline>) getNode(task_LIST_VIEW_ID);
    }
     ...
     */
    public boolean isListMatching(Deadline... tasks) {
     ...
     */
    public boolean containsInOrder(int startPosition, Deadline... tasks) {
        List<Deadline> tasksInList = getListView().getItems();
     ...
     * @param tasks A list of task in the correct order.
     */
    public boolean isListMatching(int startPosition, Deadline... tasks) throws IllegalArgumentException {
        ...
        for (int i = 0; i < tasks.length; i++) {
            ...
            guiRobot.sleep(200);
            if (!TestUtil.compareCardAndDeadline(getDeadlineCardHandle(startPosition + i), (Deadline) tasks[i])) {
                return false;
            }
        }
        return true;
    ...
    public TaskCardHandle navigateTotask(ReadOnlyTask readOnlyTask) {
        guiRobot.sleep(500); //Allow a bit of time for the list to be updated
        final Optional<Deadline> task = getListView().getItems().stream().filter(p -> p.getName().name.equals(readOnlyTask)).findAny();
        if (!task.isPresent()) {
     ...
     */
    public DeadlineCardHandle navigateToDeadline(Deadline Deadline) {
    ...
    public DeadlineCardHandle getDeadlineCardHandle(Deadline Deadline) {
