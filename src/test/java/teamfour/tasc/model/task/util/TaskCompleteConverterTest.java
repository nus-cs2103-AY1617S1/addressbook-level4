...
public class TaskCompleteConverterTest {
     ...
     * 2. completed
     *     - [no sub-partitions]
     *     (EP: Does not matter if the completed tasks have a deadline,
     *     period or even recurrence. As long as it is completed, converter should
     *     always throw error)
     * - uncompleted
     *     3. deadline & period with no recurrence
     *     4. deadline only with 1 recurrence
     *     5. period only with 2 recurrence
     *     (Heuristics: No need to test period with 1 recurrence or deadline with 2 recurrence,
     *     because whether there is deadline or period only has nothing to do with recurrence
     *     count)
     *     (Boundary: 0, 1, >1)
     *     (Heuristics: No need to test > 2, not going to be different)
     *     
     *     (Heuristics: Only test weekly. This is because testing for daily and
     *     monthly should be done by a different test (the test case responsible
     *     for testing Recurrence.getNextDateAfterRecurrence())
     */
    ...
    public void constructor_nullInput_throwsException()
        ...
        TaskCompleteConverter uselessConverter = new TaskCompleteConverter(null);
    }
    ...
    public void constructor_completedTask_throwsException()
        ...
        TaskCompleteConverter converter = new TaskCompleteConverter(completedTask);
    }
    ...
    public void constructor_deadlineAndPeriodButNoRecurrence_returnsCompletedTaskOnly()
        ...
        TestTask noRecurrenceTaskCompleted = new TaskBuilder().withName("No recurrence")
                .withDeadline(firstDayOfDecember)
                .withPeriod(firstDayOfDecember3am, firstDayOfDecember5am)
                .withCompleteStatus(true)
                .build();

        TaskCompleteConverter converter = new TaskCompleteConverter(noRecurrenceTask);
    ...
    public void constructor_deadlineWithOneRecurrence_returnsCompletedAndRemaining()
        ...
        TestTask oneRecurrenceTask = new TaskBuilder().withName("One recurrence")
                .withDeadline(firstDayOfDecember)
                .withRecurrence(Recurrence.Pattern.WEEKLY, 1)
                .build();
        
        TestTask oneRecurrenceTaskCompleted = new TaskBuilder().withName("One recurrence")
                .withDeadline(firstDayOfDecember)
        ...
        TestTask remainingTask = new TaskBuilder().withName("One recurrence")
                ...
                // no recurrence left
                .withCompleteStatus(false)
                .build();

        TaskCompleteConverter converter = new TaskCompleteConverter(oneRecurrenceTask);
    ...
    public void constructor_periodWithTwoRecurrences_returnsCompletedAndRemaining()
        ...
        TestTask twoRecurrenceTask = new TaskBuilder().withName("Two recurrences")
                .withPeriod(firstDayOfDecember3am, firstDayOfDecember5am)
                .withRecurrence(Recurrence.Pattern.WEEKLY, 2)
                .build();
        
        TestTask twoRecurrenceTaskCompleted = new TaskBuilder().withName("Two recurrences")
                .withPeriod(firstDayOfDecember3am, firstDayOfDecember5am)
                // no recurrence for completed
                .withCompleteStatus(true)
                .build();
        ...
        TaskCompleteConverter converter = new TaskCompleteConverter(twoRecurrenceTask);
