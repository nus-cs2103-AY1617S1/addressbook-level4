...
public class DateTimeUtil {
   ...
   * 
   * @@author A0139924W
   * @return String[] with first index being the startDate time and second index being the end date
   *         time
   */
  ...
  public static boolean isWithinWeek(LocalDateTime endDateTime) {
    if (endDateTime == null) {
      ...
      LocalDateTime endThisWeek = today.with(TemporalAdjusters.next(DayOfWeek.MONDAY));
   ...
   * i.e. dateTimeQuery startDateTime is equals to or before the dateTimeSource endDateTime
   ...
   * i.e. dateTimeQuery endDateTime occurs after the dateTimeSource startDateTime
   * && dateTimeQuery startDateTime occurs before the dateTimeSource endDateTime
   * 
   */
  ...
  public static String getDayAndDateString(DateTime dateTime) {
    ...
    sb.append(dateTime.getEndDate().getDayOfWeek().getDisplayName(TextStyle.FULL, Locale.ENGLISH))
        .append(StringUtil.STRING_COMMA)
  ...
  public static String getStringOfFreeDateTimeInDate(DateTime dateToCheck,
    ...
    for (DateTime dt : listOfFreeTimeSlotsInDate) {
      sb.append(StringUtil.STRING_NEWLINE).append(counter).append(StringUtil.STRING_FULLSTOP)
          .append(dt.getStartDate().format(stringFormatterWithoutDate)).append("hrs to ")
          .append(dt.getEndDate().format(stringFormatterWithoutDate)).append("hrs (")
  ...
  public static LocalDateTime setLocalTime(LocalDateTime dateTime, int hour, int min, int sec) {
    return LocalDateTime.of(dateTime.getYear(), dateTime.getMonth(), dateTime.getDayOfMonth(), hour,
        min, sec);
  }
}
