...
public class FreeCommand extends Command {
  ...
  public FreeCommand(DateTime dateToCheck) {
    ...
    this.dateToCheck
        .setStartDateTime(dateToCheck.getEndDate().withHour(0).withMinute(0).withSecond(0));
    this.dateToCheck
        .setEndDateTime(dateToCheck.getEndDate().withHour(23).withMinute(59)    );
  }
  ...
  public CommandResult execute() {
    ...
    ArrayList<DateTime> listOfFreeTimeSlots =
        DateTimeUtil.getListOfFreeTimeSlotsInDate(dateToCheck, listOfFilledTimeSlots);

    model.updateFilteredTaskListUsingDate(dateToCheck);

    if (listOfFilledTimeSlots.isEmpty()) {
      return new CommandResult(
          String.format(MESSAGE_FREE_DAY, DateTimeUtil.getDayAndDateString(dateToCheck)));
