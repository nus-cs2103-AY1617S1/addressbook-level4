...
public class UniqueTaskList implements Iterable<Task> {
	...
	public boolean isOverlapping(Task toAdd) {
		...
		for (Task i: internalList) {
			if(!i.equals(toAdd)){
				if(i.isEvent()){
					boolean cond1 = toAdd.getStartTime().compareTo(i.getEndTime()) < 0;
					boolean cond2 = toAdd.getEndTime().compareTo(i.getStartTime()) > 0;
					if(cond1 && cond2){
						return true;
					}
				}
	...
	public boolean setIncomplete(ReadOnlyTask task) throws TaskNotFoundException {
		...
		for (Task i: internalList){
			if(i.getUniqueID()==task.getUniqueID()){
				i.markAsIncomplete();
				i.updateRecurringDate(i.getStartTime().time, i.getRecurringFrequency(), -1);
				i.updateRecurringDate(i.getEndTime().time, i.getRecurringFrequency(), -1);
				return true;
			}
		}
		throw new TaskNotFoundException();
