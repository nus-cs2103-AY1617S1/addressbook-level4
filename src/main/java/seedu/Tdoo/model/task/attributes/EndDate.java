...
/**
 * Represents a Event's end date in the TodoList. Guarantees: immutable; is
 * valid as declared in {@link #isValidDate(String)}
 */
public class EndDate {

	public static final String MESSAGE_DATE_CONSTRAINTS = "Event end date should be written in this format 'DD-MM-YYYY' and date must be current or later date\n"
	...
	public static boolean isValidDate(String date) {
		...
		Calendar calendar = new GregorianCalendar(Integer.parseInt(dateArr[0]), Integer.parseInt(dateArr[1]),
		...
		// Check if input month is less than current month or more than 12
		// months
		...
		// Check if input day is more than current day or more than days in
		// current month
		else if ((Integer.parseInt(dateArr[1]) == Integer.parseInt(curDate[1])
				&& Integer.parseInt(dateArr[0]) < Integer.parseInt(curDate[0]))
				|| Integer.parseInt(dateArr[0]) > daysOfcurrMonth) {
			checkDay = false;
		}
	...
	public boolean equals(Object other) {
		return other == this // short circuit if same object
				|| (other instanceof EndDate // instanceof handles nulls
						&& this.endDate.equals(((EndDate) other).endDate)); // state
																			// check
	}
