...
//@@author A0139277U
/**
 * This class does a background check on Menion for any Activities which may
 * have it's deadline passed.
 */
public class BackgroundDateCheck {
    ...
    private static void checkTasks(Model model, Calendar currentTime) {
        ...
        for (int i = 0; i < taskList.size(); i++) {
            ...
            // Yet to send email due to no internet connection. But task
            // deadline has passed
            ...
            if (!taskToCheck.isTimePassed()
                    && taskToCheck.getActivityStatus().toString().equals(Completed.UNCOMPLETED_ACTIVITY)) {
    ...
    /**
     * This method scans through Menion to find any events which may have
     * passed.
     * 
     * @param model
     * @param currentTime
     ...
     *            date
     * @param dateValues[0]:
     *            day in months, dateValues[1]: month, dateValues[2]: year
     */
