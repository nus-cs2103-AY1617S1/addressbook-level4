...
public class Task implements ReadOnlyTask, Comparable<Task> {
	...
	// this(source.getName(), source.getTaskDescription(), source.getDate(),
	// source.getStartTime(), source.getEndTime(), source.getTags());
	// }
			...
			if (this.getDate().getFullDate() != "") {
				if (this.getDate().compareTo(new Date(DATEFORMATTER.format(calendar.getTime()))) < 0) {
					...
					if (this.getEndTime().getFullTime() != "") {
						if (Integer.parseInt(this.getEndTime().getFullTime())
								- Integer.parseInt(TIMEFORMATTER.format(calendar.getTime())) < 0) {
							return true;
						}
					}
				}
